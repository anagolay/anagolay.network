...
<nav class='flex items-center justify-between bg-main w-full text-sm py-5'>
    ...
    <div class="flex">
        <div class={`flex flex-col absolute top-16 bottom-0  ${toggleMenu?'w-full py-auto translate-x-full duration-300':''} py-5  text-center right-full bg-main mx-auto 
            md:flex-row md:mx-0 md:relative md:top-auto md:h-full md:py-0 md:right-auto md:left-auto md:w-fit md:translate-x-0`}>
            {#each links as link}
                <a href={link.url} class='px-2 my-2 md:my-auto min-w-fit hover:underline' class:active={$page.url.pathname === link.url} on:click={toggleSideBar}>
                    {link.caption}
                </a>
            {/each}
            <button class="bg-secondary px-6 my-2 p-2 w-2/4 mx-auto rounded-full flex items-center justify-center
                md:my-1 md:py-1 md:w-24 md:mx-5" on:click={toggleSideBar}>
                FAQ
            </button>
        </div>
        <button class='flex p-1 mx-2 bg-secondary rounded-md'
